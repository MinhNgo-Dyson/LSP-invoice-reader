...
def process_invoice(pdf_path, poppler_path=None, tesseract_path=None):
    ...
    if lsp == "ceva":
        ...
        total_amount_usd, total_amount_myr, exchange_rate = extract_total_amount_ceva(
            lines)
    elif lsp == "dhl":
        invoice_number = extract_invoice_number_dhl(lines)
        total_amount_usd, total_amount_myr, exchange_rate = extract_total_amount_dhl(
            lines)
    elif lsp == "maersk":
        invoice_number = extract_invoice_number_msk(lines)
        total_amount_usd, total_amount_myr, exchange_rate = extract_total_amount_msk(
            lines)
    else:
        return {
            'lsp': "Unknown",
        ...
        for filename in files:
            if filename.lower().endswith('.pdf'):
                ...
                result = process_invoice(
                    pdf_path, poppler_path, tesseract_path)
...
if __name__ == "__main__":
    try:
        folder_path = input("What is the invoice folder?").replace(
            "\"", "").replace("\'", "")
        # Start the timer
        start_time = time.time()
        print("Processing invoices ...")
        results = process_invoices_in_folder(
            folder_path, POPPLER_PATH, TESSERACT_PATH)
        # End the timer
        end_time = time.time()
        ...
        output_folder_path = input("What is the export folder?").replace(
            "\"", "").replace("\'", "")
